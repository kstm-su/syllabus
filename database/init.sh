...
mysql -uroot -p$PASSWORD $DB_NAME -e"GRANT ALL ON $DB_NAME.* FOR \`$ADMIN_USER\`" > /dev/null 2>&1
mysql -uroot -p$PASSWORD $DB_NAME -e"GRANT SELECT ON $DB_NAME.* FOR \`$GUEST_USER\`" > /dev/null 2>&1
